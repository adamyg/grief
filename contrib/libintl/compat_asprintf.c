...
__CIDENT_RCSID(gr_compat_asprintf,"$Id: compat_asprintf.c,v 1.1 2024/06/04 13:17:24 cvsuser Exp $")
...
#undef libintl_asprintf
#undef libintl_vasprintf

int
libintl_asprintf(char **str, const char *fmt, ...)
{
	va_list ap;
	int size;

	va_start(ap, fmt);
	size = libintl_vasprintf(str, fmt, ap);
	va_end(ap);
...
int
libintl_vasprintf(char **str, const char *fmt, va_list ap)
{
	va_list tap;
	char *buf = NULL;
	int osize, size;
