 ...
 *
 ...
 Conversion                 Explanation                                 Expected Argument Type
 Specifier
                                                        Length
                                                        Modifier        hh      h       none    l       ll      j       z       t       L

 %      writes literal %. The full conversion specification
        must be %%.                                                     N/A     N/A     N/A     N/A     N/A     N/A     N/A     N/A     N/A

 c      writes a single character.                                      N/A     N/A     N/A     N/A     N/A     N/A     N/A     N/A     N/A

 s      writes a character string                                       N/A     N/A     char    wchar   N/A     N/A     N/A     N/A     N/A

 S      writes a character string                                       N/A     N/A     WChar   N/A     N/A     N/A     N/A     N/A     N/A

 d,i    converts a signed integer into decimal "[-]dddd".               signed  short   int     long    long    intmax  size_t  ptrdiff N/A
                                                                        char                            long

                                                                        unsigned
 o      converts an unsigned integer into octal                         signed  short   int     long    long    intmax  size_t  ptrdiff N/A
        representation oooo.                                            char                            long

 x,X    converts an unsigned integer into hexadecimal                   :       :       :       :       :       :       :       :       :
        representation hhhh.

        For the x conversion letters abcdef are used.
        For the X conversion letters ABCDEF are used.

 u      converts an unsigned integer into decimal "dddd".               :       :       :       :       :       :       :       :       :

 f,f    converts floating-point number to the decimal notation          N/A     N/A     double  double  N/A     N/A     N/A     N/A     N/A
        in the style [-]ddd.ddd.

 e,E    converts floating-point number to the decimal                   :       :       :       :       :       :       :       :       :
        exponent notation.

        For the e conversion style [-]d.ddde±dd is used.
        For the E conversion style [-]d.dddE±dd is used.

 a,A    converts floating-point number to the hexadecimal               :       :       :       :       :       :       :       :       :
        exponent notation.  -- NOT SUPPORTED --

        For the a conversion style [-]0xh.hhhp±d is used.
        For the A conversion style [-]0Xh.hhhP±d is used.

 g,G    converts floating-point number to decimal or decimal            :       :       :       :       :       :       :       :       :
 ...
 n      returns the number of characters written so far by this         signed  short   int     long    long    intmax  size_t  ptrdiff N/A
        call to the function.                                           char *                          long
 ...
 p      writes an implementation defined character sequence             N/A     N/A     void*   N/A     N/A     N/A     N/A     N/A     N/A
        defining a pointer.

 */

/*
 * Source: https://github.com/weiss/c99-snprintf
 */
 ...
 *    	#include <stdarg.h>
 *    	#if !HAVE_VSNPRINTF
 *    	int rpl_vsnprintf(WChar_t *, size_t, const WChar_t *, va_list);
 *    	#endif
 *    	#if !HAVE_SNPRINTF
 *    	int rpl_snprintf(WChar_t *, size_t, const WChar_t *, ...);
 *    	#endif
 *    	#if !HAVE_VASPRINTF
 *    	int rpl_vasprintf(WChar_t **, const WChar_t *, va_list);
 *    	#endif
 *    	#if !HAVE_ASPRINTF
 *    	int rpl_asprintf(WChar_t **, const WChar_t *, ...);
 *    	#endif
 *    	#endif
 *
...
#if !HAVE_WSNPRINTF || !HAVE_WVSNPRINTF || !HAVE_WASPRINTF || !HAVE_WVASPRINTF
...
static void fmtstrW(WChar_t *, size_t *, size_t, const wchar_t *, int, int, int);
static void fmtstr(WChar_t *, size_t *, size_t, const char *, int, int, int);
static void fmtint(WChar_t *, size_t *, size_t, INTMAX_T, int, int, int, int);
static void fmtflt(WChar_t *, size_t *, size_t, LDOUBLE, int, int, int, int *);
static void printsep(WChar_t *, size_t *, size_t);
static int getnumsep(int);
static int getexponent(LDOUBLE);
static int convert(UINTMAX_T, WChar_t *, size_t, int, int);
...
int
Wvsnprintf(WChar_t *str, size_t size, const char *format, va_list args)
{
	LDOUBLE fvalue;
	INTMAX_T value;
	/*unsigned*/ WChar_t cvalue;
		...
		case PRINT_S_CONV:
			...
			case 'p':
				...
				if ((strvalue = va_arg(args, void *)) == NULL)
					 ...
					 * "0x0", SysV "0".
					 */
					fmtstrw(str, &len, size, nil, width, -1, flags);
				else {
					/*
...
static void
fmtstrw(WChar_t *str, size_t *len, size_t size, const WChar_t *value, int width, int precision, int flags)
{
...
static void
fmtstr(WChar_t *str, size_t *len, size_t size, const char *value, int width, int precision, int flags)
{
	int padlen, strln;	/* Amount to pad. */
	int noprecision = (precision == -1);
...
static void
fmtint(WChar_t *str, size_t *len, size_t size, INTMAX_T value, int base, int width,
       int precision, int flags)
{
	UINTMAX_T uvalue;
	WChar_t iconvert[MAX_CONVERT_LENGTH];
	WChar_t sign = 0;
	WChar_t hexprefix = 0;
...
static void
fmtflt(WChar_t *str, size_t *len, size_t size, LDOUBLE fvalue, int width, int precision, int flags, int *overflow)
{
	LDOUBLE ufvalue;
	UINTMAX_T intpart;
	UINTMAX_T fracpart;
	UINTMAX_T mask;
	const WChar_t *infnan = NULL;
	WChar_t iconvert[MAX_CONVERT_LENGTH];
	WChar_t fconvert[MAX_CONVERT_LENGTH];
	WChar_t econvert[5];	/* "e-300" (without nul-termination). */
	WChar_t esign = 0;
	WChar_t sign = 0;
	int leadfraczeros = 0;
	int exponent = 0;
	...
	if (infnan != NULL) {
		...
		fmtstrw(str, len, size, iconvert, width, ipos, flags);
		return;
	}
...
static void
printsep(WChar_t *str, size_t *len, size_t size)
{
...
static int
convert(UINTMAX_T value, WChar_t *buf, size_t size, int base, int caps)
{
	const WChar_t *digits = caps ? HEX : hex;
	size_t pos = 0;
...
int
Wvasprintf(WChar_t **ret, const char *format, va_list ap)
{
	size_t size;
	int len;
	va_list aq;
	...
	VA_COPY(aq, ap);
	len = Wvsnprintf(NULL, 0, format, aq);
	VA_END_COPY(aq);
	if (len < 0 || (*ret = malloc(size = len + 1)) == NULL)
		return -1;
	return Wvsnprintf(*ret, size, format, ap);
}
#endif	/* !HAVE_WVASPRINTF */
...
#if HAVE_STDARG_H
int
Wsnprintf(WChar_t *str, size_t size, const char *format, ...)
#else
int
Wsnprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	WChar_t *str;
	...
	VA_START(ap, format);
	VA_SHIFT(ap, str, WChar_t *);
	VA_SHIFT(ap, size, size_t);
	VA_SHIFT(ap, format, const WChar_t *);
	len = Wvsnprintf(str, size, format, ap);
	va_end(ap);
	return len;
}
#endif	/* !HAVE_WSNPRINTF */

#if !HAVE_WASPRINTF
#if HAVE_STDARG_H
int
Wasprintf(WChar_t **ret, const char *format, ...)
#else
int
Wasprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	WChar_t **ret;
	...
	VA_START(ap, format);
	VA_SHIFT(ap, ret, WChar_t **);
	VA_SHIFT(ap, format, const WChar_t *);
	len = Wvasprintf(ret, format, ap);
	va_end(ap);
	return len;
}
#endif	/* !HAVE_WASPRINTF */
#else	/* Dummy declaration to avoid empty translation unit warnings. */
int main(int argc, WChar_t **argv);
#endif	/* !HAVE_WSNPRINTF || !HAVE_WVSNPRINTF || !HAVE_WASPRINTF || [...] */

#if TEST_SNPRINTF
int
main(void)
{
	const WChar_t *float_fmt[] = {
		/* "%E" and "%e" formats. */
	...
	const WChar_t *pointer_fmt[] = {
		"foo|%p|bar",
		"%42p",
		"%p",
		NULL
	};
	const WChar_t *pointer_val[] = {
		*pointer_fmt,
		*string_fmt,
		*string_val,
		NULL
