...
/* $Id: ncx.cr,v 1.1 2014/11/16 17:27:44 ayoung Exp $
...
/*
 *  ncx ---
 *      Directory services (Norton/Midnight commander style).
 */
void
ncx(~string)
{
    ...
    if (get_parm(0, url, "URL: ") <= 0 || "" == url) {
        return;
    }

    if (-1 == (ncconn = ftp_create()) ||
            -1 == ftp_connect(ncconn, url)) {
        ftp_close(ncconn);
        return;
    }
     ...
     *  Load selected files
     */
    if (-1 == ncconn && strlen(ncfilebuf)) {    /* F10 or Esc ? */
        if (ncfilebuf == MARKER) {
            /*
...
formatsize(int bytes)
{
    if (bytes < 9999999) {
        ...
        return format("%dK", bytes);
    } else if ((bytes /= 1024) < 999999) {
        return format("%dM", bytes);
    }
    return format("%dG", bytes/1024);
}
...
debug(1);
        ftp_directory(ncconn, NULL, files);
        while (list_each(files, name) >= 0) {
            ftp_stat(ncconn, name, bytes, mtime, NULL, mode);

            if ((0 == mode && -1 == bytes) || (S_IFDIR & mode)) {
                if (0 == mode) mode = S_IFDIR|0777;
                sprintf(buffer, " /%-*.*s|  <DIR>|", ncfwidth, ncfwidth, name);

            } else {                            /* non-directory */
                if (0 == mode) mode = S_IFREG;
                sprintf(buffer, " %s%-*.*s|%7s|", substr(mode_string(mode, cwd+"/"+name, TRUE), 1, 1), 
                    ncfwidth, ncfwidth, name, formatsize(bytes));
            }
