...
__CIDENT_RCSID(gr_mac1_c,"$Id: mac1.c,v 1.76 2023/01/02 09:12:04 cvsuser Exp $")

/* -*- mode: c; indent-width: 4; -*- */
/* $Id: mac1.c,v 1.76 2023/01/02 09:12:04 cvsuser Exp $
 * Basic primitives.
 *
 *
...
int                     x_selfinsert = FALSE;   /* Utilised by the undo collapser. */
 ...
 *
 *<<GRIEF>>
    Macro: __lexicalblock - Lexical scope.

        __lexicalblock( .... block ... );

    Macro Description:
        The '__lexicalblock' statement is used to implement lexical block scoping
        within function/macro bodies.

        By default local variables are all function scoped unless lexical block
        scoping is enabled via the #pragma lexical_scoping.
...
>       #pragma scope(pop)
        ...
        Note!:
        The '__lexicalblock()' primitive is *internal* to the macro language.
        It is generated by the GRIEF macro compiler when nested variable
        declarations are encountered within block statements.
 ...
 */
void
do_lexicalblock(void)           /* ( ... statements ... ) */
{
    trace_ilog("__lexicalblock\n");
    assert(isa_list(1));
    if (isa_list(1)) {
        sym_local_build();                      /* new block scope */
        execute_macro(get_list(1));
        sym_local_delete(FALSE);                /* exit inner block */
    }
}
 ...
 */
    ...
    errorf("foreach not implemented.");
}
...
//          /*
//           *  Make sure there are > 1 reference to the result.
//           *      If there's only one ref, it may be the accumulators yet as
//           *      eval doesn't tell us how it derived the*  result and we could screw up badly if we dont do this.
//           */
//          r_inc(result.l_ref);
                                         ...
                                         [compile], [cmversion], [features], [build]) */
{
    ...
    */
    if (isa_undef(1)) {
        ewprintf("%s %s.%s compiled %s", x_appname, x_version, x_buildnumber, x_compiled);
        return;
    }
...
do_grief_version(void)
{
    ...
    if (isa_undef(1)) {
        ret = 102;
            /* 1.2 - 1/04/2020, register variables and lexical scoping */

    } else {
        switch (get_xinteger(1, 0)) {
        case 1:             /* build-type */
#if defined(NDEBUG)
            ret = 1;                            /* release */
#else
...
do_returns(int returns)         /* (expr) */
{
    if (returns) {                              /* returns; */
        ...
        if (margc >= 2) {       /* is optional, 1/4/2020 */
            acc_assign_argv(margv + 1);
        } else {
            acc_assign_null();  /* void/implied NULL */
        }
        set_return();
    }
}
...
do_unimp(void)                  /* void (...) */
{
    ewprintf("*** <%s> not yet implemented.", execute_name());
}
