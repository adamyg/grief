...
static int              rgbvalue_new(const char **cursor, int rgbmax);
...
static const struct rgbdef xterm_basic16[16] = {
    { 0,    0x00, 0x00, 0x00 },
    { 1,    0xCD, 0x00, 0x00 },
    { 2,    0x00, 0xCD, 0x00 },
    { 3,    0xCD, 0xCD, 0x00 },
...
static const struct rgbdef win_basic16[16] = {
    { 0,    0x00, 0x00, 0x00 },
    { 1,    0x00, 0x00, 0x80 },
    { 2,    0x00, 0x80, 0x00 },
    { 3,    0x00, 0x80, 0x80 },
...
rgb_import(const char *name, int length, struct rgbvalue *rgb, int rgbmax)
{
    ...
    if (name && length > 0) {
        if (0 == strncmp(name, "rgb:", 4)) {
            ...
            r = rgbvalue_new(&cursor, rgbmax);
            g = rgbvalue_new(&cursor, rgbmax);
            b = rgbvalue_new(&cursor, rgbmax);
            if (b >= 0 && 0 == *cursor) {
                ret = COLORRGB_RGB;
            }
            ...
            switch (--length) {
            case 3:         /* 4 bits   */
                r = rgbvalue_old(&cursor, 1, rgbmax);
                g = rgbvalue_old(&cursor, 1, rgbmax);
                b = rgbvalue_old(&cursor, 1, rgbmax);
                break;
            case 6:         /* 8 bits   */
                r = rgbvalue_old(&cursor, 2, rgbmax);
                g = rgbvalue_old(&cursor, 2, rgbmax);
                b = rgbvalue_old(&cursor, 2, rgbmax);
                break;
            case 9:         /* 12 bits  */
                r = rgbvalue_old(&cursor, 3, rgbmax);
                g = rgbvalue_old(&cursor, 3, rgbmax);
                b = rgbvalue_old(&cursor, 3, rgbmax);
                break;
            case 12:        /* 16 bits  */
                r = rgbvalue_old(&cursor, 4, rgbmax);
                g = rgbvalue_old(&cursor, 4, rgbmax);
                b = rgbvalue_old(&cursor, 4, rgbmax);
                break;
            default:
                break;
            }
 ...
 */
static int
rgbvalue_new(const char **cursor, int rgbmax)
{
...
{
    return rgb_searchtable(rgb_win256table(), 256, 24, rgb);
}
...
{
    return rgb_searchtable(rgb_xterm256table(), 256, 24, rgb);
}
...
rgb_searchtable(const struct rgbdef *table, int maxval, int greyscale, const struct rgbvalue *rgb)
{
     ...
     */
    if (greyscale > 0 && red == green && red == blue) {
        minval  = maxval - greyscale;           /* grey-scale table */
        table  += minval;
...
main(int argc, char *argv[])
{
    ...
    for (cnt = i = 0; NULL != (p = specs[i]); ++i) {
        if (*p) {
            ...
            printf("%02d: %-32s (%2d) == %2d/%02x/%02x/%02x [%s]\n",
